    ...
    internal class DataRepositoryTests {
        ...
        public void TestFindMethodCalls_Nested() {
            using(var sa = new SrcMLArchive("DataRepositoryTests")) {
                ...
                using(var da = new DataRepository(sa)) {
                    da.InitializeData();
                    var foo = da.GlobalScope.ChildScopes.OfType<IMethodDefinition>().First(md => md.Name == "Foo");
                    var expected = new[]
                               {
        ...
        public void TestFindMethodCalls_Simple() {
            using(var sa = new SrcMLArchive("DataRepositoryTests")) {
                ...
                using(var da = new DataRepository(sa)) {
                    da.InitializeData();
                    var expected = da.GlobalScope.ChildScopes.OfType<IMethodDefinition>().First(md => md.Name == "main").MethodCalls.First();
        ...
        public void TestFindScopeForAdjacentMethods() {
            using(var sa = new SrcMLArchive("DataRepositoryTests")) {
                ...
                using(var da = new DataRepository(sa)) {
                    da.InitializeData();
                    var mainMethod = da.GlobalScope.GetDescendantScopes<IMethodDefinition>().FirstOrDefault();
                    var fooMethod = da.GlobalScope.GetDescendantScopes<IMethodDefinition>().LastOrDefault();
